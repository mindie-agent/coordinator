#!/usr/bin/env python3
"""Host/container bootstrap scripts owned by mindie-coordinator.
...
IMAGE_RESOLVER_USER_AGENT = "mindie-machine-management/1.0"
...
SENTINEL = "__MINDIE_JSON__="
DEFAULT_PASSWORD_ENV = "MINDIE_SSH_PASSWORD"
...
def _mindie_image_ref_repo(ref):
...
def _mindie_image_ref_machine_type(ref):
    if "@" in ref:
        return None
...
def _mindie_local_ref_rank(ref):
    is_digest = "@" in ref
    tag = ""
    if not is_digest:
        last_slash = ref.rfind("/")
    ...
    for image in image_inspects:
        ...
        for ref in list(image.get("RepoTags") or []) + list(image.get("RepoDigests") or []):
            if not isinstance(ref, str) or not ref.strip():
                continue
            repo = _mindie_image_ref_repo(ref.strip())
            if "vllm-ascend" not in repo.rsplit("/", 1)[-1].lower():
                continue
            references.append(ref.strip())
        references = sorted(set(references), key=_mindie_local_ref_rank)
        if not references:
            continue
        compatible_references = [
            ref
            for ref in references
            if machine_type is None or _mindie_image_ref_machine_type(ref) == machine_type
        ]
        entry = {
...
def run_with_askpass(
    ...
    *,
    ...
    env_name = validate_env_name(env_name)
    temp_dir = pathlib.Path(tempfile.mkdtemp(prefix="mindie-askpass-"))
    try:
        ...
        env.setdefault("DISPLAY", "mindie:0")
...
def format_progress_event(
    ...
    return f"[mindie{host}][{phase}] {message}"
...
safe_source /etc/profile.d/mindie-ascend-env.sh
safe_source /usr/local/Ascend/ascend-toolkit/set_env.sh
safe_source /usr/local/Ascend/ascend-toolkit/latest/set_env.sh
safe_source /usr/local/Ascend/nnal/atb/set_env.sh "--cxx_abi=${MINDIE_ATB_CXX_ABI:-1}"
...
result: dict[str, object] = {
    ...
    "npu_probe": {
        ...
        "env": {
            ...
            "MINDIE_NPU_SOC": os.environ.get("MINDIE_NPU_SOC"),
        },
    },
    "image": {
...
soc_from_env = os.environ.get("SOC_VERSION") or os.environ.get("MINDIE_NPU_SOC")
...
prepared_image_tag() {
  ...
  printf 'mindie-session-prepared:%s-ssh-v2\n' "$image_hash"
}
...
write_host_state() {
  ...
  host_soc="$3"
  host_image="$4"
  install -d -m 0755 /etc/mindie /etc/profile.d
...
if [ -n "$host_machine_type" ]; then
  export MINDIE_MACHINE_TYPE="$host_machine_type"
fi
if [ -n "$host_container_type" ]; then
  export MINDIE_CONTAINER_TYPE="$host_container_type"
fi
if [ -n "$host_soc" ]; then
  export MINDIE_NPU_SOC="$host_soc"
  export SOC_VERSION="$host_soc"
fi
if [ -n "$host_image" ]; then
  export MINDIE_CONTAINER_IMAGE="$host_image"
fi
EOF_HOST_ENV
...
set -euo pipefail
state_file=/tmp/mindie-package-bootstrap.json
rm -f "$state_file"
...
PY
  ...
  _mindie_runtime_python=""
  for _d in $(ls -1d /usr/local/python*/bin 2>/dev/null | sort -V -r); do
    if [ -x "$_d/python3" ]; then
      _mindie_runtime_python="$_d/python3"
      break
    fi
  done
  if [ -z "$_mindie_runtime_python" ] && command -v python3 >/dev/null 2>&1; then
    _mindie_runtime_python="$(command -v python3)"
  fi
  if [ -n "$_mindie_runtime_python" ]; then
...
EOF_PIP
    ...
    echo "[mindie-bootstrap] pip configured: /etc/pip.conf (index=huaweicloud)"
  else
    echo "[mindie-bootstrap] runtime python not found, skipping pip setup"
  fi
INNER_PKG
}
...
install -d -m 0755 /run/sshd /etc/profile.d /etc/mindie
_mindie_python_dir=""
for _d in $(ls -1d /usr/local/python*/bin 2>/dev/null | sort -V -r); do
  if [ -x "$_d/python3" ]; then
    _mindie_python_dir="$_d"
    break
  fi
done
if [ -n "$_mindie_python_dir" ]; then
  _path_prefix="$_mindie_python_dir:/usr/local/bin:/usr/local/sbin"
else
  _path_prefix="/usr/local/bin:/usr/local/sbin"
fi
_mindie_atb_cxx_abi="1"
if [ -n "$_mindie_python_dir" ] && [ -x "$_mindie_python_dir/python3" ]; then
  _torch_cxx_abi="$("$_mindie_python_dir/python3" - <<'PY' 2>/dev/null || true
import torch
...
PY
  ...
  case "$_torch_cxx_abi" in
    0|1)
      _mindie_atb_cxx_abi="$_torch_cxx_abi"
      ;;
  esac
fi
patch_atb_set_env_source() {
  file="$1"
  [ -f "$file" ] || return 0
  if [ ! -f "${file}.mindie-atb-abi.bak" ]; then
    cp -a "$file" "${file}.mindie-atb-abi.bak" 2>/dev/null || true
  fi
  tmp_file="$(mktemp)"
  if awk -v abi="$_mindie_atb_cxx_abi" '
...
cat > /etc/profile.d/mindie-ascend-env.sh <<EOF_CONTAINER_ENV
#!/bin/sh
export MINDIE_ATB_CXX_ABI="\${MINDIE_ATB_CXX_ABI:-$_mindie_atb_cxx_abi}"
if [ -z "\${ASCEND_HOME_PATH:-}" ]; then
  if [ -d /usr/local/Ascend/ascend-toolkit/latest ]; then
    export ASCEND_HOME_PATH=/usr/local/Ascend/ascend-toolkit/latest
  elif [ -d /usr/local/Ascend/ascend-toolkit ]; then
...
if [ -f /usr/local/Ascend/nnal/atb/set_env.sh ]; then
  . /usr/local/Ascend/nnal/atb/set_env.sh "--cxx_abi=\${MINDIE_ATB_CXX_ABI:-1}" >/dev/null 2>&1 || true
fi
export LD_LIBRARY_PATH="/usr/local/Ascend/driver/lib64/common:/usr/local/Ascend/driver/lib64/driver:/usr/local/Ascend/driver/lib64:\${LD_LIBRARY_PATH:-}"
export PATH="$_path_prefix:\${PATH:-}"
if [ -n "$machine_type" ]; then
  export MINDIE_MACHINE_TYPE="$machine_type"
fi
if [ -n "$container_type" ]; then
  export MINDIE_CONTAINER_TYPE="$container_type"
fi
if [ -n "$soc" ]; then
  export MINDIE_NPU_SOC="$soc"
  export SOC_VERSION="$soc"
fi
if [ -n "$image" ]; then
  export MINDIE_CONTAINER_IMAGE="$image"
fi
if [ -n "$visible_devices" ]; then
  export ASCEND_RT_VISIBLE_DEVICES="$visible_devices"
fi
EOF_CONTAINER_ENV
chmod 0644 /etc/profile.d/mindie-ascend-env.sh
python3 - "$machine_type" "$container_type" "$soc" "$image" "$workdir" "$namespace" "$_mindie_atb_cxx_abi" "$visible_devices" > /etc/mindie/container-info.json <<'PY'
import json
import sys
...
print(json.dumps({
    ...
    "env_file": "/etc/profile.d/mindie-ascend-env.sh",
}, ensure_ascii=False, indent=2))
PY
chmod 0644 /etc/mindie/container-info.json
_mindie_runtime_python=""
if [ -n "$_mindie_python_dir" ] && [ -x "$_mindie_python_dir/python3" ]; then
  _mindie_runtime_python="$_mindie_python_dir/python3"
...
cat > /etc/ssh/sshd_mindie_config <<EOF_SSH
Port ${port}
...
PidFile /run/sshd_mindie.pid
EOF_SSH
if [ -n "$visible_devices" ]; then
  printf 'SetEnv ASCEND_RT_VISIBLE_DEVICES=%s\n' "$visible_devices" >> /etc/ssh/sshd_mindie_config
fi
/usr/sbin/sshd -t -f /etc/ssh/sshd_mindie_config
if [ -f /run/sshd_mindie.pid ]; then
  kill "$(cat /run/sshd_mindie.pid)" 2>/dev/null || true
  rm -f /run/sshd_mindie.pid
fi
pkill -f '/etc/ssh/sshd_mindie_config' 2>/dev/null || true
/usr/sbin/sshd -f /etc/ssh/sshd_mindie_config
...
host_env_file="/etc/profile.d/mindie-ascend-env.sh"
host_metadata_file="/etc/mindie/host-info.json"
container_env_file="/etc/profile.d/mindie-ascend-env.sh"
container_metadata_file="/etc/mindie/container-info.json"
...
docker_pull_policy="${MINDIE_DOCKER_PULL_POLICY:-missing}"
...
if docker inspect "$container" >/dev/null 2>&1; then
  ...
  current_base_image=$(docker inspect -f '{{ index .Config.Labels "com.mindie.base_image" }}' "$container" 2>/dev/null || true)
  ...
  if [ "$image_policy" = "__LOCAL_IMAGE_DISCOVERY_POLICY__" ]; then
    ...
    recorded_base_image_id="$(docker inspect -f '{{ index .Config.Labels "com.mindie.base_image_id" }}' "$container" 2>/dev/null || true)"
...
if [ "$container_exists" = "true" ]; then
  ...
  existing_base_image=$(docker inspect -f '{{ index .Config.Labels "com.mindie.base_image" }}' "$container" 2>/dev/null || true)
  ...
  if [ "$use_prepared_image_cache" = "true" ] && [ -n "$existing_base_image" ]; then
    ...
    prepared_image="$(docker inspect -f '{{ index .Config.Labels "com.mindie.prepared_image" }}' "$container" 2>/dev/null || true)"
  ...
  if [ -n "$visible_devices" ]; then
    ...
    visibility_args+=("--label" "com.mindie.visible_devices=$visible_devices")
  fi

  emit_progress "container" "running" "creating managed container" 45
  docker run --name "$container" -it -d --network host --shm-size=500g \
    --privileged=true \
    --label com.mindie.managed=true \
    --label com.mindie.container_ssh_port="$port" \
    --label com.mindie.workdir="$workdir" \
    --label com.mindie.namespace="$namespace" \
    --label com.mindie.machine_type="$machine_type" \
    --label com.mindie.container_type="$machine_type" \
    --label com.mindie.soc="$soc" \
    -w "$workdir" \
    ...
    "${mount_args[@]}" \
    "${visibility_args[@]}" \
    --label com.mindie.base_image="$selected_image" \
    --label com.mindie.base_image_id="$selected_image_id" \
    --label com.mindie.run_image="$run_image" \
    --label com.mindie.prepared_image="$prepared_image" \
    "$run_image" >/dev/null
...
if ! docker exec "$container" bash -lc 'command -v sshd >/dev/null 2>&1 && command -v ssh >/dev/null 2>&1'; then
  ...
  package_bootstrap="$(docker exec "$container" bash -lc 'cat /tmp/mindie-package-bootstrap.json 2>/dev/null || true')"
  docker exec "$container" bash -lc 'rm -f /tmp/mindie-package-bootstrap.json' >/dev/null 2>&1 || true
...
def build_parser() -> argparse.ArgumentParser:
    ...
    probe.add_argument(
        "--managed-prefix",
        default="mindie-",
        help="managed container name prefix (default: mindie-)",
    )
    probe.add_argument(
        "--machine-type",
    ...
    mesh_export.add_argument(
        "--comment",
        required=True,
        help="stable mesh key comment, for example mindie-mesh:173.125.1.2",
    )
    ...
    mesh_remove.add_argument(
        "--peer-comment",
        required=True,
        help="stable mesh comment, for example mindie-mesh:173.125.1.2",
    )
    mesh_remove.add_argument(
