...
from mindie_coordinator.managed_execution import ExecutionRequestError, JOB_TERMINAL, ManagedExecution
from mindie_coordinator.runtime_profile import digest
from mindie_coordinator.run_manifest import utc_now
...
class RuntimePool(ManagedExecution):
    ...
    def event(self, db, owner, kind, **data):
        ...
        get_recorder("mindie-coordinator").event("DEBUG", "pool." + kind,
    ...
    def _checkout_row(self, owner, session, key, runtime, observed, *, requested_runtime):
        ...
        return {"id": key, "owner": owner,
                ...
                "environment": {"MINDIE_ENVIRONMENT_FINGERPRINT": profile_key},
    ...
    def export_execution_record(self, run, binding, job=None):
        ...
        record = {
            "schema_version": "mindie.managed-run.v2",
            "run_id": run["id"],
    ...
    def _run_operation(self, run, operation):
        ...
        try:
            with get_recorder("mindie-coordinator").operation("run." + operation, run_id=run["id"]):
                yield
        finally:
            ...
            try:
                ...
                get_recorder("mindie-coordinator").event("WARNING", "diagnostics.persistence_failed",
        ...
        with self._entity_lock("run", run_id):
            ...
            try:
                ...
                if service_port is not None:
                    run["environment"]["MINDIE_SERVICE_PORT"] = str(int(service_port))
            ...
            for row, managed in sorted(candidates, key=lambda item: item[0]['last_poll']):
                ...
                def advance(row=row, managed=managed, lock=lock):
                    try:
                        ...
                        lock.release()
                threading.Thread(target=advance, name='mindie-pool-' + row['id'][:12], daemon=True).start()
                started += 1
                if started >= limit:
                    break
            return
    ...
    def runtime_busy(self, runtime_id: str, *, except_job: str | None = None) -> bool:
        with self.transaction() as db:
            from mindie_coordinator.managed_execution import JOB_TERMINAL
            bindings = [row for row in self.rows(db, "binding")
